...
static void k210_dvp_sccb_clk_init(void)
{
    uint32_t tmp;
...
uint32_t k210_dvp_sccb_set_clk_rate(uint32_t clk_rate)
{
    uint32_t tmp;
...
static void k210_dvp_sccb_start_transfer(void)
{
...
void k210_dvp_sccb_send_data(uint8_t dev_addr, uint16_t reg_addr, uint8_t reg_data)
{
    uint32_t tmp;
    ...
    k210_dvp_sccb_start_transfer();
}

uint8_t k210_dvp_sccb_receive_data(uint8_t dev_addr, uint16_t reg_addr)
{
    uint32_t tmp;
...
static void k210_dvp_reset(void)
{
    /* First power down */
...
void k210_dvp_init(uint8_t reg_len)
{
    ...
    k210_dvp_sccb_clk_init();
    k210_dvp_reset();
}

uint32_t k210_dvp_set_xclk_rate(uint32_t xclk_rate)
{
...
void k210_dvp_set_image_format(uint32_t format)
{
    uint32_t tmp;
...
void k210_dvp_enable_burst(void)
{
...
void k210_dvp_disable_burst(void)
{
    dvp->dvp_cfg &= (~DVP_CFG_BURST_SIZE_4BEATS);
...
void k210_dvp_set_image_size(uint32_t width, uint32_t height)
{
    uint32_t tmp;
...
void k210_dvp_set_ai_addr(uint32_t r_addr, uint32_t g_addr, uint32_t b_addr)
{
    dvp->r_addr = r_addr;
    dvp->g_addr = g_addr;
    dvp->b_addr = b_addr;
...
void k210_dvp_set_display_addr(uint32_t addr)
{
    dvp->rgb_addr = addr;
}

void k210_dvp_start_frame(void)
{
...
void k210_dvp_start_convert(void)
{
    dvp->sts = DVP_STS_DVP_EN | DVP_STS_DVP_EN_WE;
}

void k210_dvp_finish_convert(void)
{
...
void k210_dvp_config_interrupt(uint32_t interrupt, uint8_t enable)
{
    if (enable)
...
int k210_dvp_get_interrupt(uint32_t interrupt)
{
...
void k210_dvp_clear_interrupt(uint32_t interrupt)
{
    interrupt |= (interrupt << 1);
    dvp->sts |= interrupt;
}

void k210_dvp_enable_auto(void)
{
    dvp->dvp_cfg |= DVP_CFG_AUTO_ENABLE;
}

void k210_dvp_disable_auto(void)
{
    dvp->dvp_cfg &= (~DVP_CFG_AUTO_ENABLE);
}
