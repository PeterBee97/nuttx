...
static void dmac_enable(void)
{
    ...
    dmac_cfg.data = getreg64(&dmac->cfg);
    dmac_cfg.cfg.dmac_en = 1;
    dmac_cfg.cfg.int_en = 1;
    putreg64(dmac_cfg.data, &dmac->cfg);
}
...
void src_transaction_complete_int_enable(dmac_channel_number_t channel_num)
{
    ...
    ch_intstat.data = getreg64(&dmac->channel[channel_num].intstatus_en);
    ch_intstat.ch_intstatus_enable.enable_src_transcomp_intstat = 1;

    putreg64(ch_intstat.data, &dmac->channel[channel_num].intstatus_en);
}
...
int32_t dmac_check_channel_busy(dmac_channel_number_t channel_num)
{
    ...
    chen_u.data = getreg64(&dmac->chen);
    switch(channel_num)
    {
        case DMAC_CHANNEL0:
    ...
    putreg64(chen_u.data, &dmac->chen);
                                    ...
                                    dmac_src_dst_select_t sd_sel, dmac_master_number_t mst_num)
{
    ...
    ctl.data = getreg64(&dmac->channel[channel_num].ctl);
    ...
    if(ret == 0)
    {
        ...
        putreg64(ctl.data, &dmac->channel[channel_num].ctl);
    }
...
void dmac_enable_common_interrupt_status(void)
{
    ...
    intstatus.data = getreg64(&dmac->com_intstatus_en);
    ...
    putreg64(intstatus.data, &dmac->com_intstatus_en);
}
...
void dmac_enable_common_interrupt_signal(void)
{
    ...
    intsignal.data = getreg64(&dmac->com_intsignal_en);
    ...
    putreg64(intsignal.data, &dmac->com_intsignal_en);
}

static void dmac_enable_channel_interrupt(dmac_channel_number_t channel_num)
{
    putreg64(0xffffffff, &dmac->channel[channel_num].intclear);
    putreg64(0x2, &dmac->channel[channel_num].intstatus_en);
}

void dmac_disable_channel_interrupt(dmac_channel_number_t channel_num)
{
    putreg64(0, &dmac->channel[channel_num].intstatus_en);
}
                            ...
                            dmac_channel_config_t *cfg_param)
{
     ...
     * sar dar register
     */
    cfg.data = getreg64(&dmac->channel[channel_num].cfg);
    ...
    putreg64(cfg.data, &dmac->channel[channel_num].cfg);
    ...
    putreg64(cfg_param->ctl_block_ts, &dmac->channel[channel_num].block_ts);
    ...
    ch_llp.data = getreg64(&dmac->channel[channel_num].llp);
    ch_llp.llp.loc = cfg_param->llp_loc;
    ch_llp.llp.lms = cfg_param->llp_lms;
    putreg64(ch_llp.data, &dmac->channel[channel_num].llp);
    putreg64(ctl.data, &dmac->channel[channel_num].ctl);
    getreg64(&dmac->channel[channel_num].swhssrc);
                           ...
                           uint32_t blockSize)
{
     ...
     * sar dar register
     */
    cfg_u.data = getreg64(&dmac->channel[channel_num].cfg);
    ...
    putreg64(cfg_u.data, &dmac->channel[channel_num].cfg);
    ...
    putreg64(blockSize - 1, &dmac->channel[channel_num].block_ts);
    /*the number of (blcok_ts +1) data of width SRC_TR_WIDTF to be */
    /* transferred in a dma block transfer */
    return 0;
}
                            ...
                            dmac_channel_config_t *cfg_param)
{
    ...
    cfg_param->ctl_block_ts = getreg64(&dmac->channel[channel_num].block_ts);
...
void dmac_set_address(dmac_channel_number_t channel_num, uint64_t src_addr,
                      uint64_t dst_addr)
{
    putreg64(src_addr, &dmac->channel[channel_num].sar);
    putreg64(dst_addr, &dmac->channel[channel_num].dar);
}
                       ...
                       uint32_t block_size)
{
    ...
    putreg64(block_ts, &dmac->channel[channel_num].block_ts);
}
                         ...
                         dmac_burst_trans_length_t burst_length)
{
    ...
    ctl_u.data = getreg64(&dmac->channel[channel_num].ctl);
    ...
    putreg64(ctl_u.data, &dmac->channel[channel_num].ctl);
}
                                     ...
                                     uint64_t *addr)
{
    ...
    llp_u.data = getreg64(&dmac->channel[channel_num].llp);
    /* Cast pointer to uint64_t */
    llp_u.llp.loc = (uint64_t)addr;
    putreg64(llp_u.data, &dmac->channel[channel_num].llp);
}
...
void dmac_init(void)
{
    ...
    dmac_reset.data = getreg64(&dmac->reset);
    dmac_reset.reset.rst = 1;
    putreg64(dmac_reset.data, &dmac->reset);
    while(dmac_reset.reset.rst)
        dmac_reset.data = getreg64(&dmac->reset);

    /*reset dmac */

    intclear.data = getreg64(&dmac->com_intclear);
    ...
    putreg64(intclear.data, &dmac->com_intclear);
    ...
    while(getreg64(&dmac->cfg))
        ;
    tmp = getreg64(&dmac->chen);
    tmp &= ~0xf;
    putreg64(tmp, &dmac->chen);
                                  ...
                                  int8_t last_block, dmac_channel_config_t *cfg_param)
{
    ...
    putreg64(cfg_param->sar, &dmac->channel[channel_num].sar);
    putreg64(cfg_param->dar, &dmac->channel[channel_num].dar);
    putreg64(cfg_param->ctl_block_ts, &dmac->channel[channel_num].block_ts);
    ...
    putreg64(ctl_u.data, &dmac->channel[channel_num].ctl);
    putreg64(0, &dmac->channel[channel_num].blk_tfr);
}
...
{
    if(getreg64(&dmac->channel[channel_num].intstatus) & 0x2)
        return 1;
    else
        return 0;
}
...
static int dmac_irq_callback(int irq, FAR void *ctx, FAR void *arg)
{
    ...
    if(v_dmac_context->callback != NULL)
        v_dmac_context->callback(irq, v_dmac_context->ctx, arg);

    return 0;
}

#define IRQN_DMA0_INTERRUPT 27
#define PLIC_PRIO_BASE K210_PLIC_BASE

void dmac_irq_register(dmac_channel_number_t channel_num, xcpt_t dmac_callback, void *ctx, uint32_t priority)
{
