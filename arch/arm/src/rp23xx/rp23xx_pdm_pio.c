...
int rp23xx_pdm_pio_configure(int mode, uint32_t samplerate)
{
  ...
  uint32_t clock_pin_base = CONFIG_RP23XX_PDM_CLOCK;
  ...
  rp23xx_pio_sm_set_pindirs_with_mask(g_pdm_pio, g_pdm_pio_sm,
                                      pin_mask, pin_mask);
  rp23xx_pio_sm_set_pins(g_pdm_pio, g_pdm_pio_sm, 1); /* clear pins */
...
{
  return RP23XX_PIO_TXF(g_pdm_pio, g_pdm_pio_sm);
}
...
{
  return RP23XX_DMA_DREQ_PIO0_TX0 + g_pdm_pio_sm + g_pdm_pio * 8;
}
